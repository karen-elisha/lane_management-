...
def recognize_text(image):
    ...
    text = pytesseract.image_to_string(enhanced_image)  # Use pytesseract to extract text
    print("Recognized Text:", text.strip())  # Print the recognized text
    return text.strip()
...
def take_photo(frame):
    ...
    return recognize_text(inverted_frame)
...
def main():
    ...
    while True:
        ...
        if (hand_status["left_hand_detected"] and hand_status["right_hand_detected"]):
            ...
            print(f"Recognized text from photo: {text}")
